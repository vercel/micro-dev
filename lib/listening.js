...
const restartServer = (file, flags, watcher, cbStarted) => {
  ...
  // Restart the server
  require('./serve')(file, flags, true, cbStarted)
}
...
module.exports = async (server, inUse, flags, sockets, cbStarted) => {
  ...
  if (!flags.cold) {
    ...
    watcher.once(
      ...
      debounce((event, filePath) => {
        ...
        // Restart server
        server.close(restartServer.bind(this, file, flags, watcher, cbStarted))
      }, 10)
    )
  }

  // XXX: pre-callback or post-callback?
  if (cbStarted) {
    cbStarted(server)
  }
