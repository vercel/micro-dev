...
const restartServer = (file, flags, watcher, cbBeforeStart) => {
  ...
  // Restart the server
  require('./serve')(file, flags, true, cbBeforeStart)
}
...
module.exports = async (server, inUse, flags, sockets, cbBeforeStart) => {
  ...
  if (!flags.cold) {
    ...
    watcher.once(
      ...
      debounce((event, filePath) => {
        ...
        // Restart server
        server.close(restartServer.bind(this, file, flags, watcher, cbBeforeStart))
      }, 10)
    )
  }
