...
const restartServer = (file, flags, watcher) => {
  ...
  for (const mainPath in watched) {
    ...
    for (const subPath of subPaths) {
      const full = join(mainPath, subPath)
      toDelete.push(full)
    }
  }
...
module.exports = async (server, inUse, flags, sockets) => {
  ...
  const closestPkg = await pkgUp(dirname(file))
  const toWatch = closestPkg ? dirname(closestPkg) : process.cwd()
  ...
  watcher.on('all', (event, filePath) => {
    const location = relative(process.cwd(), filePath)
    console.log(
